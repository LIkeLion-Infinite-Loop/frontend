...
//   const headerTitleColor = isDarkMode ? '#FFFFFF' : '#000000';
//   const categorySectionBackgroundColor = isDarkMode ? '#1E1E1E' : 'rgba(247, 247, 247, 0.3)';
//   const categoryTitleColor = isDarkMode ? '#05D16E' : '#06D16E';
//   const categoryTitleBorderColor = isDarkMode ? '#444444' : '#DBDBDB';
//   const itemCardBorderColor = isDarkMode ? '#282828' : '#ffffffff';
//   const itemIconBackgroundColor = isDarkMode ? '#333333' : 'rgba(255, 255, 255, 0.3)';
//   const itemNameColor = isDarkMode ? '#E0E0E0' : '#333333';
//   const itemDescriptionColor = isDarkMode ? '#BBBBBB' : '#666666';


//   return (
//     <View style={[styles.container, { backgroundColor: containerBackgroundColor }]}>
//       <Text style={[styles.headerTitle, { color: headerTitleColor }]}>재활용 분리배출 가이드</Text>
      
//       <ScrollView style={styles.scrollView}>
//         {Object.keys(RECYCLING_DATA).map((categoryKey) => {
//           const category = RECYCLING_DATA[categoryKey];
//           return (
//             <View key={categoryKey} style={[styles.categorySection, { backgroundColor: categorySectionBackgroundColor }]}>
//               <Text style={[styles.categoryTitle, { color: categoryTitleColor, borderBottomColor: categoryTitleBorderColor }]}>{category.koreanName}</Text>
//               <View style={styles.itemsContainer}>
//                 {category.items.map((item: RecyclingItem) => (
//                   <View key={item.name} style={[styles.itemCard, { borderBottomColor: itemCardBorderColor }]}>
//                     <Image source={item.icon} style={[styles.itemIcon, { backgroundColor: itemIconBackgroundColor }]} />
//                     <View style={styles.itemDetails}>
//                       <Text style={[styles.itemName, { color: itemNameColor }]}>{item.name}</Text>
//                       <Text style={[styles.itemDescription, { color: itemDescriptionColor }]}>{item.description}</Text>
//                     </View>
//                   </View>
//                 ))}
//               </View>
